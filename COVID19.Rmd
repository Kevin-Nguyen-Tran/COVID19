---
title: "COVID-19 Analysis - Florida, California, and Texas"
author: "Kevin Tran"
date: "8/27/2020"
...
This is a analysis on COVID-19 and exploring the positive cases/trends, visualizing the difference in positive and negative cases, and inspecting death trends. California, Florida, and Texas are the top 3 states with the most positive cases in the U.S. We will compare Florida to Texas and California to analyze average % increase of deaths per day and average deaths per day. The data represented in this analysis can be found on <https://covidtracking.com/> and will cover the time frame of 2020-03-04 to 2020-08-07.
...
* The difference in positive and negative cases
...
**Line chart to show positive vs negative cases in Florida:**
```{r}
...
California, Florida, and Texas are the top 3 states with the most positive cases in the U.S. Due to similar sample sizes, we will compare Florida to these states to analyze average % increase of deaths and average deaths per day to see if they differ statistically.
...
```
The output tell us that the p-value is close to zero, therefore we can reject our null hypothesis that there is no difference in average deaths. The difference within means fall between 767.51 to 1929.88 and since zero is not within the confidence interval as well, we can reject the null hypothesis with 95% confidence.
...
## Florida vs Texas
We will first start by comparing the average difference in deaths between Florida and Texas. To do so, we will conduct an Independent 2-sample t-test and generate a confidence interval to see if Florida and Texas' average deaths differ.

This t-test will tell us how significant the average difference of deaths are between Florida and Texas.
...
*We will utilize the above averages in a ANOVA test to compare statistical significance, between all three states, in the next section!*

## Florida vs California vs Texas
We will compare the data for California, Florida, and Texas in regards to average percent increase in deaths per day and average deaths per day.
We will visualize via table and visualize the percent increase of death via scatter plot. Lastly, we will compare all 3 states in regards to the above two factors to test statistical significance with an ANOVA test!
...
**We will fit a non-linear regression for the relationship between Y = death rate, X1 = date, and X2 = state (TX, FL and CA)
...
We can run an ANOVA (Analysis of Variance) test to see if the average % increase of deaths per day are equal (not statistically significant) within the 3 states.

If they are not equal we can run further tests to see which one(s) are different.

* Our Null Hypothesis (Ho) is: The average percent increase of deaths per day between the 3 states are not significantly different.
...
Our p-value is 57.9%. Therefore, we fail to reject our null hypothesis. This means that the average percent increase of deaths per day between the 3 states are not significantly different.
...
**Next, we can run an ANOVA test to see if the average deaths per day are equal (not statistically significant) within the 3 states.**
